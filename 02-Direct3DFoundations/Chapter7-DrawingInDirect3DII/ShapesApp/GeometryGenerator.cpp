...
void GeometryGenerator::BuildCylinderTopCap(float radius, float height, uint32 sliceCount, MeshData& meshData)
{
...
MeshData GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, uint32 sliceCount, uint32 stackCount)
{
	...
	for (uint32 i = 0; i < ringCount; i++)
	{
		...
		for (uint32 j = 0; j <= sliceCount; j++)
		{
			...
			vertex.UV = XMFLOAT2(static_cast<float>(j) / sliceCount, 1.0f - static_cast<float>(i) / sliceCount);
	...
	for (uint32 i = 0; i < stackCount; i++)
	{
		for (uint32 j = 0; j < sliceCount; j++)
		{
			meshData.Indices32.push_back(i*ringCount + j);
			meshData.Indices32.push_back((i+1)*ringCount + j);
...
MeshData GeometryGenerator::CreateGeoSphere(float radius, uint32 numberOfSubdivisions)
{
	MeshData meshData;
	
	numberOfSubdivisions = std::min<uint32>(numberOfSubdivisions, 6u);

	const float x = 0.525731f;
	const float z = 0.850651f;

	XMFLOAT3 positions[12] =
	{
	...
	for (uint32 i = 0; i < numberOfSubdivisions; i++)
	{
		Subdivide(meshData);
	}
	...
	for (uint32 i = 0; i < meshData.Vertices.size(); i++)
	{
		...
		XMVECTOR projection = radius * normal;

		XMStoreFloat3(&meshData.Vertices[i].Position, projection);
		...
		meshData.Vertices[i].UV = XMFLOAT2(theta / XM_PI, phi / XM_PI);
		meshData.Vertices[i].Tangent = XMFLOAT3(-radius*sinf(phi) * sinf(theta), 0.0f, radius*sinf(phi)*cosf(theta));
...
MeshData GeometryGenerator::CreateBox(float width, float height, float depth, uint32 numSubdivisions) const
{
	MeshData meshData;
